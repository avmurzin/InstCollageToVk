...
/**
 * Реализация интерфейса {@link GetInstagramData} доступа к Инстаграм путем обращения к официальному
 * API.
 *
 ...
 * @version 0.1
 */
public class GetInstagramDataByApi implements GetInstagramData {
    static final String ACCESS_TOKEN = "3413030827.d9abaab.01aeb9d269af4d6f8f04e8fa1c0d4327";

    private ArrayList<InstagramPost> postList;

    public GetInstagramDataByApi() {
    ...
    public ArrayList<InstagramPost> getTopPostList(String username, int listSize) {
        ...
        //получение user_id по указанному имени
        try {
            dataJson = new JSONObject(getJSON("https://api.instagram.com/v1/users/search?q=" + username + "&access_token=" + ACCESS_TOKEN));
            JSONArray data = dataJson.getJSONArray("data");
            JSONObject dataItem = data.getJSONObject(0);
            userId = dataItem.getString("id");
        ...
        while (moreAvailable) {
            try {
                dataJson = new JSONObject(getJSON("https://api.instagram.com/v1/users/" + userId + "/media/recent/?access_token=" + ACCESS_TOKEN +"&max_id=" + max_id));

                JSONArray items = dataJson.getJSONArray("data");
    ...
    /**
     * Получение номера элемента в списке объектов {@link InstagramPost}, имеющего лайков меньше,
     * чем у объекта, заданного параметром post.
     * @param post Объект для сравнения
     * @param postList Список, в котором производится поиск
     * @return Номер найденного элемента (-1, если не найден)
     */
